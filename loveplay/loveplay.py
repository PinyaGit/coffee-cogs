...
class Loveplay(commands.Cog):
    ...
    async def lpanal(self, ctx, *, user):
        """Send a anal"""
        desc = "anal"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/anal/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed

    @commands.hybrid_command(name="blowjob")
    @commands.is_nsfw()
    async def lpblowjob(self, ctx, *, user):
        """Send a blowjob"""
        desc = "blowjob"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/blowjob/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed
    ...
    async def lpcum(self, ctx, *, user):
        """Send a cum"""
        desc = "cum"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/cum/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed
        
    @commands.hybrid_command(name="pussylick")
    @commands.is_nsfw()
    async def lppussylick(self, ctx, *, user):
        """Send a pussylick"""
        desc = "pussylick"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/pussylick/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed

    @commands.hybrid_command(name="solo")
    @commands.is_nsfw()
    async def lpsolo(self, ctx, *, user):
        """Send a solo"""
        desc = "solo"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/solo/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed

    @commands.hybrid_command(name="solo_male")
    @commands.is_nsfw()
    async def lpsolo_male(self, ctx, *, user):
        """Send a solo_male"""
        desc = "solo_male"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/solo_male/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed

    @commands.hybrid_command(name="threesome_fff")
    @commands.is_nsfw()
    async def lpthreesome_fff(self, ctx, *, user):
        """Send a threesome_fff"""
        desc = "threesome_fff"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/threesome_fff/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed

    @commands.hybrid_command(name="threesome_ffm")
    @commands.is_nsfw()
    async def lpthreesome_ffm(self, ctx, *, user):
        """Send a threesome_ffm"""
        desc = "threesome_ffm"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/threesome_ffm/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed

    @commands.hybrid_command(name="threesome_mmf")
    @commands.is_nsfw()
    async def lpthreesome_mmf(self, ctx, *, user):
        """Send a threesome_mmf"""
        desc = "threesome_mmf"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/threesome_mmf/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed

    @commands.hybrid_command(name="yaoi")
    @commands.is_nsfw()
    async def lpyaoi(self, ctx, *, user):
        """Send a yaoi"""
        desc = "yaoi"
        req = await self.fetch_url("https://purrbot.site/api/img/nsfw/yaoi/gif")
        src = req.get("link", "") # Silently fail if no image returned
        e = await self.buildEmbed(ctx, desc, src, user)
        return # Sent in buildEmbed
